-- Red Squadron Armory V52 Safe Automatic Discord Role Sync
-- Fixes V51 schema mismatch and prevents access wipeouts during failed Discord sync.
...
as $$
begin
  -- Safety guard: if the role sync returned no roles, do NOT remove editor access.
  -- Empty roles can happen during Discord API/bot failures. This prevents Command,
  -- S4, or Admin from losing panel access during a bad sync.
  ...
  -- Editor roles should unlock all armories.
...
-- Rebuild policies around the stable helper.
...
-- Restore editor rows from any already-detected roles without manual usernames.
insert into public.user_access (user_id, access_key)
