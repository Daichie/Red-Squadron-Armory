-- Red Squadron Armory V48 Editor Role ID RLS Failsafe
-- Run after V45. This keeps publishing automatic for Discord Command/S4/Admin roles.
-- It allows editor access if either:
-- 1) public.user_access contains admin/command/s4_rd, OR
-- 2) public.profiles.last_detected_roles contains a configured editor Discord role ID.
...
-- Known editor role IDs supplied by Red Squadron Command.
...
-- If you have a separate Discord Admin role, add it here later:
-- insert into public.armory_editor_roles (access_key, discord_role_id, label, is_active)
-- values ('admin', 'PASTE_ADMIN_ROLE_ID_HERE', 'Admin', true)
...
-- Rebuild core RLS policies using the failsafe helper.
