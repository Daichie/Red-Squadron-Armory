...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Always load the existing profile row and prefer its stored Discord ID.
    // Supabase Auth metadata can vary between OAuth/session flows; using the
    // profile row prevents accidental Discord member lookups with the wrong ID.
    if (!profileFromDb) {
      const { data: currentProfile } = await admin
        ...
        .eq("id", user.id)
        .maybeSingle();
      profileFromDb = currentProfile || null;
    }
    ...
    if (discordRes.status === 404) {
      // Do not wipe existing access on a Discord lookup failure. This prevents
      // Command/S4/Admin users from losing access because of a temporary bot,
      // guild, or Discord API issue.
      ...
      return jsonResponse({
        ok: false,
        error: "Discord member lookup returned 404. Existing access was preserved.",
        discord_user_id: String(discordUserId),
        access_keys: null,
      }, 403);
    }
    ...
    if (!Array.isArray(member.roles)) {
      return jsonResponse({
        ok: false,
        error: "Discord member response did not include a roles array. Existing access was preserved.",
        discord_user_id: String(discordUserId),
        raw_member_keys: Object.keys(member || {}),
      }, 500);
    }
    ...
    // IMPORTANT: keep this update aligned with the deployed profiles schema.
    // Your production table currently has last_detected_roles and last_role_sync,
    // but not last_detected_role_names / last_detected_editor_keys.
    ...
    if (profileUpdateError) {
      console.error("profiles upsert failed:", profileUpdateError);
      return jsonResponse({
        ok: false,
        error: "Failed to update profiles during Discord role sync",
        details: profileUpdateError.message,
      }, 500);
    }

    const { error: deleteAccessError } = await admin.from("user_access").delete().eq("user_id", user.id);
    if (deleteAccessError) {
      console.error("user_access delete failed:", deleteAccessError);
      return jsonResponse({
        ok: false,
        error: "Failed to refresh user access rows",
    ...
    if (rows.length) {
      const { error: insertAccessError } = await admin.from("user_access").insert(rows);
      if (insertAccessError) {
        console.error("user_access insert failed:", insertAccessError);
        return jsonResponse({
          ok: false,
          error: "Failed to insert user access rows",
          details: insertAccessError.message,
          access_keys_attempted: accessArray,
        }, 500);
      }
    }

    // Audit logging must never block role sync. Some older deployments may not
    // have access_audit_logs or may have different RLS/policies.
    try {
      ...
      console.warn("access_audit_logs insert skipped:", auditError);
    }
