...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const requestApiKey = req.headers.get("apikey") || req.headers.get("Apikey") || "";

    if (!jwt) return jsonResponse({ ok: false, error: "Missing Authorization bearer token" }, 401);
    if (!requestApiKey) return jsonResponse({ ok: false, error: "Missing request apikey header" }, 401);

    const decoded = decodeJwtPayload(jwt);
    const supabaseUserId = decoded?.sub ? String(decoded.sub) : "";
    if (!supabaseUserId) return jsonResponse({ ok: false, error: "Invalid Supabase user token: missing subject" }, 401);

    // IMPORTANT V56:
    // Do not depend on SERVICE_ROLE_KEY here. Your project had an invalid service key secret.
    // This function now writes through a SECURITY DEFINER RPC using the logged-in user's JWT.
    ...
    if (existingProfileError) {
      return jsonResponse({
        ok: false,
        error: "Could not read logged-in profile before Discord role sync",
        details: existingProfileError.message,
        hint: "Run V56 SQL first. It must allow authenticated users to read profiles and execute apply_discord_role_sync."
      }, 200);
    }

    const discordUserId = String(existingProfile?.discord_user_id || "").trim();
    const discordUsername = String(existingProfile?.discord_username || existingProfile?.display_name || "Discord User");

    if (!discordUserId) {
      return jsonResponse({
        ok: false,
        error: "No Discord user ID found in profile",
        user_id: supabaseUserId,
        hint: "Logout/login through Discord again so the frontend can save discord_user_id into profiles before role sync."
      }, 200);
    }
    ...
    if (!botLookup.ok && providerToken) {
      ...
      memberLookup = providerLookup.ok ? providerLookup : {
        ...providerLookup,
        bot_error: { status: botLookup.status, details: botLookup.text }
      };
    }

    if (!memberLookup.ok) {
      const syncMessage = `SYNC_FAILED:${memberLookup.source}:status=${memberLookup.status}:${memberLookup.text || "No response body"}`;
      await userClient.rpc("apply_discord_role_sync", {
        p_discord_user_id: discordUserId,
        p_discord_username: discordUsername,
        p_display_name: discordUsername,
        p_main_branch: existingProfile?.main_branch || "general",
        p_role_ids: [],
        p_role_names: [syncMessage],
        p_editor_keys: existingProfile?.last_detected_editor_keys || [],
        p_access_keys: []
      });

      return jsonResponse({
        ok: false,
        error: "Discord member lookup failed. Existing access was preserved where possible.",
    ...
    // Load role map from secrets and public armory_editor_roles table.
    const roleMap: Record<string, string> = {};
    addMappedRole(roleMap, "ROLE_GENERAL", "general");
    addMappedRole(roleMap, "ROLE_SRC", "src");
    addMappedRole(roleMap, "ROLE_LRR", "lrr");
    ...
    const { data: editorRoleRows } = await userClient
      .from("armory_editor_roles")
      .select("access_key, discord_role_id, is_active")
      .eq("is_active", true);
    if (Array.isArray(editorRoleRows)) {
    ...
    if (accessKeys.has("admin") || accessKeys.has("command") || accessKeys.has("s4_rd")) {
      ["general", "src", "lrr", "kalasag", "haribon", "uav_operator", "marksman", "combat_medic", "rto", "admin", "command", "s4_rd"]
    ...
    const { error: rpcError } = await userClient.rpc("apply_discord_role_sync", {
      p_discord_user_id: discordUserId,
      p_discord_username: discordUsername,
      p_display_name: discordUsername,
      p_main_branch: mainBranch,
      p_role_ids: roleIds,
      p_role_names: detectedRoleNames.length ? detectedRoleNames : [`SYNC_OK_NO_NAMED_ROLES:${memberLookup.source}`],
      p_editor_keys: editorKeys,
      p_access_keys: accessArray
    });

    if (rpcError) {
      return jsonResponse({
        ok: false,
        error: "Failed to apply Discord role sync through RPC",
        details: rpcError.message,
        hint: "Run supabase/v56_user_jwt_role_sync_rpc.sql in SQL Editor, then redeploy this function."
      }, 200);
    }
    ...
    return jsonResponse({
      ...
      note: "V56 synced through authenticated SECURITY DEFINER RPC; SERVICE_ROLE_KEY is no longer required for database writes."
    }, 200);
  } catch (error) {
