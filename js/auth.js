const RS_AUTH_VERSION = "V54_PROVIDER_TOKEN_ROLE_SYNC";
...
function rsStoredAccessToken() {
  ...
  if (expiresAt && Date.now() > expiresAt) {
    ...
    // Keep cached user/access keys; Supabase may still restore a refreshed session.
    return null;
  }
  ...
  try {
    ...
    if (accessToken && refreshToken) {
      ...
      if (!error && data && data.session) {
        ...
        if (data.session.provider_token) localStorage.setItem("rsArmoryDiscordProviderToken", data.session.provider_token);
    ...
    if (accessToken) {
      rsStoreAccessToken(accessToken, expiresIn, expiresAt);
      const providerTokenFromHash = hash.get("provider_token");
      if (providerTokenFromHash) localStorage.setItem("rsArmoryDiscordProviderToken", providerTokenFromHash);
...
async function rsGetSession() {
  ...
  // V53: Prefer Supabase's persisted session first. Supabase stores the refresh
  // token in localStorage and can refresh the session automatically. This keeps
  // users logged in after closing/reopening the website until they click Sign Out.
  if (client) {
    try {
      const { data, error } = await client.auth.getSession();
      if (!error && data && data.session && data.session.user) {
        ...
        if (data.session.provider_token) localStorage.setItem("rsArmoryDiscordProviderToken", data.session.provider_token);
        localStorage.setItem("rsArmorySupabaseUser", JSON.stringify(data.session.user));
        return data.session;
      }
    } catch (error) {
      console.warn("Stored Supabase session check failed:", error);
    }
  }
...
async function rsSignOut() {
  ...
  localStorage.removeItem("rsArmoryDiscordProviderToken");
...
async function rsSyncDiscordRoles() {
  ...
  if (!accessToken) {
    console.warn("Discord role sync skipped: no Supabase access token found.");
    return null;
  }

  try {
    const response = await fetch(`${RS_SUPABASE_CONFIG.url}/functions/v1/sync-discord-roles`, {
      method: "POST",
      headers: {
        "Authorization": `Bearer ${accessToken}`,
        "apikey": RS_SUPABASE_CONFIG.publishableKey,
      ...
      body: JSON.stringify({ source: "red-squadron-armory-web", discord_provider_token: localStorage.getItem("rsArmoryDiscordProviderToken") || null })
    });
    ...
    if (!response.ok) {
      console.warn("Discord role sync failed:", response.status, data);
      return data;
    }
    ...
    console.log("Discord role sync result:", data);
    return data;
  } catch (error) {
    console.warn("Discord role sync request failed:", error);
    return null;
  }
}
...
async function rsGetAllowedSections() {
  ...
  if (error) {
    console.warn("Could not load user access keys:", error.message);
    return [];
  }
