...
async function loadAdminArmoryItems() {
  ...
  if (!data || !data.length) {
    list.innerHTML = '<p class="muted">No items posted yet for this armory. Use the form to add the first live item.</p>';
    return;
  }
...
async function deleteArmoryItem() {
  const id = document.getElementById('itemId').value;
  if (!id) return;
  if (!confirm('Delete this live armory item?')) return;
