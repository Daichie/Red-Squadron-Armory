...
function renderDetailFields(type, values = {}) {
  ...
  wrap.innerHTML = fields.map(([key, label]) => {
    ...
    const longFields = ['vest', 'backpack', 'notes', 'note', 'loadout'];
    const isLong = longFields.includes(key);
    const isWide = ['vest', 'backpack', 'notes', 'note', 'loadout', 'details'].includes(key);
    return `
...
async function saveArmoryItem(event) {
  ...
  try {
    // Re-sync immediately before publishing so Command/S4/Admin roles are written to user_access
    // before Supabase RLS policies evaluate the insert/upload.
    if (typeof rsSyncDiscordRoles === 'function') {
      const syncResult = await rsSyncDiscordRoles();
      if (syncResult && syncResult.error) {
        console.warn('Role sync warning before publish:', syncResult);
      }
    }
...
async function clearAdminLogs() {
  if (!confirm('Clear all visible admin monitor logs? This will not delete published armory items.')) return;
  const client = adminClient();
...
async function initArmoryAdminTools() {
  // Force sync on admin page load so user_access is updated from current Discord roles.
  if (typeof rsSyncDiscordRoles === 'function') {
