...
function userCanAccessUnit(unit, allowedKeys) {
  if (!allowedKeys) return true; // local preview/fallback
  if (unit.slug === 'general') return true;
  const key = accessKeyForUnit(unit);
  return allowedKeys.includes(key) || allowedKeys.includes('admin');
}
...
async function getAllowedAccessKeysForUI() {
  try {
    ...
    console.warn('Access check skipped for local preview:', error);
    return null;
  }
}
