...
function userCanAccessUnit(unit, allowedKeys) {
  ...
  return normalizedKeys.includes(key) || normalizedKeys.includes('admin') || normalizedKeys.includes('command') || normalizedKeys.includes('s4_rd');
}
...
async function rsUserHasAdminAccess() {
  ...
  // Force a fresh Discord role sync before checking admin access.
  // This prevents old cached access keys from hiding the editor after new
  // ROLE_COMMAND / ROLE_S4_RD secrets are added.
  const syncResult = await rsSyncDiscordRoles();
  if (syncResult && Array.isArray(syncResult.access_keys)) {
    return syncResult.access_keys.some(key => ["admin", "command", "s4_rd"].includes(key));
  }

  const allowed = await rsGetAllowedSections();
  if (!allowed) return false;

  return allowed.some(item => ["admin", "command", "s4_rd"].includes(item.access_key || item));
}
...
function rsUserCanSeeAdminLink() {
  ...
  try {
    ...
    return Array.isArray(keys) && keys.some(key => ["admin", "command", "s4_rd"].includes(key));
  } catch {
    return false;
  }
}
...
function rsArmoryClient() {
  const token = rsStoredAccessToken ? rsStoredAccessToken() : null;
  return token ? rsClientWithAccessToken(token) : rsGetClient();
}

function cloneArmoryUnit(unit) {
  // V42 live-editor mode: keep the armory section metadata/design,
  // but do not auto-load the old hardcoded beta content.
  // Uniforms, weapons, and utilities now come from Supabase armory_items only.
  return {
    ...unit,
    uniforms: [],
    weapons: [],
...
function normalizeDynamicArmoryItem(row) {
  const details = row.details && typeof row.details === 'object' ? row.details : {};
  const base = {
    id: row.id,
    name: row.name,
    authorized: row.authorized || details.authorized || 'Authorized personnel',
    image: row.image_url || details.image || IMG.accessory,
    dynamic: true
  };

  if (row.item_type === 'uniform') {
    return {
      ...base,
      uniformImage: row.image_url || details.uniformImage || IMG.uniform,
      wornImage: row.second_image_url || details.wornImage || IMG.worn,
      headgear: details.headgear,
      helmet_attachments: details.helmet_attachments || details.attachments || details.accessories,
      facewear: details.facewear,
      eyewear: details.eyewear,
      vest: details.vest,
      backpack: details.backpack,
      belt: details.belt,
      handwear: details.handwear,
      wristwear: details.wristwear,
      top: details.top,
      bottom: details.bottom,
      footwear: details.footwear,
      accessories: details.accessories,
      note: details.note || details.notes
    };
  }
  ...
  if (row.item_type === 'weapon') {
    return {
      ...
      details: details.details || details.attachments || '',
      notes: details.notes || '',
      loadout: details.loadout || WEAPON_WARNING
    };
  }
  ...
  return {
    ...
    notes: details.notes || details.note || ''
  };
}

async function loadDynamicArmoryUnit(unit) {
  const merged = cloneArmoryUnit(unit);
  // Local preview should also start clean. Add content through Supabase on deployed builds.
  if (RS_SUPABASE_CONFIG.enableLocalPreview) return merged;

  const client = rsArmoryClient();
  ...
  if (error) {
    console.warn('Dynamic armory load skipped:', error.message);
    return merged;
  }

  (data || []).forEach(row => {
    const item = normalizeDynamicArmoryItem(row);
    if (row.item_type === 'uniform') merged.uniforms.unshift(item);
    if (row.item_type === 'weapon') merged.weapons.unshift(item);
    if (row.item_type === 'utility') merged.utilities.unshift(item);
  });
...
async function renderArmoryPage() {
  ...
  unit = await loadDynamicArmoryUnit(unit);
...
function emptyArmoryMessage(type) {
  return `
    <div class="empty-state">
      <h3>No new ${escapeHtml(type)} posted yet</h3>
      <p>This armory section has not been updated yet.</p>
    </div>
  `;
}
...
function renderUniforms(items) {
  return `
    ...
    ${!items.length ? emptyArmoryMessage('uniforms') : `
        ...
        <article class="uniform-card">
          ...
          <div class="detail-column">
            ...
            <div class="detail-grid uniform-detail-grid">
              ...
              ${detail('Helmet Attachments', item.helmet_attachments)}
...
function renderWeapons(items) {
  if (!items.length) {
    return `
      ...
      ${emptyArmoryMessage('weapons / arsenal')}
    `;
  }
...
function renderUtilities(items) {
  return `
    ...
    ${!items.length ? emptyArmoryMessage('utilities') : `
