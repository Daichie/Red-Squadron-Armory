...
Deployed site now fails closed for role access. It no longer unlocks all armories when role sync is missing or not finished. General stays open; other armories require `user_access` / Discord role sync. Admin unlocks all.


## V37 Supabase Publishable Key Fix

Updated `js/config.js` with the current Supabase publishable key to fix `Invalid API key / 401 Unauthorized`.


## V38 Role Sync Read Fix

Removed `is_active` condition from `user_access` lookup and improved direct Edge Function role sync call/debugging.
